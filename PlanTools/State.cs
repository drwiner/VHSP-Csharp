...
namespace BoltFreezer.PlanTools
{
    ...
    public class State : IState
    {
        ...
        public State NewStateSimple(Operator action)
        {
            // Create a new state object.
            var state = this.Clone() as State;
            ...
            foreach (var eff in action.Effects)
            {
                ...
                if (eff.Sign == false)
                {
                    ...
                    // Check that reversed Effect is in state, otherwise we are re-stating something that's true.
                    if (!state.Predicates.Contains(reversedEff))
                    {
                        // This can happen but probably shouldn't.
                        continue;
                    }
                    else
                    {
                        state.Predicates.Remove(reversedEff);
                    }
